...
- 协议能力：
  ...
  - 本轮强化：`opus_experimental` 已从配置入口推进到受控实验链路；默认仍保持 PCM16。
- 诊断能力：
...
- Opus：已进入协议枚举、capabilities、服务端编码、Android 系统解码与桌面配置入口。
  - 启用条件：必须显式选择 `--data-plane v2_header --codec opus_experimental` 或在桌面端选择 `Opus experimental (V2)`。
  - 服务端：仅在有效数据面为 `v2_header` 时使用 `opus-rs` 进行实验编码；否则 effective codec 自动回退 `pcm16`。
  - Android：后台播放链路可识别 `LAV2 codec=3`，并通过系统 `MediaCodec audio/opus` 解码为 PCM16 后进入现有 jitter buffer / AudioTrack。
  - 当前限制：Opus 尚未完成真机听感与稳定性验收，不作为默认主路径，不用于声明稳定出声。
...
- 后续扩展：
  - 先控制面稳定协商，再灰度数据面 header，再做 Opus 实验验收，最后才评估默认路径切换。
...
- 注意：该版本不切默认数据面、不声明 loopback + v2 已稳定。

v31 Opus 实验链路配置（本轮）：
- 服务端新增真实 `opus_experimental` 编码路径，仅在有效数据面为 `v2_header` 时启用。
- Android 后台播放链路新增 `LAV2 codec=3` 解析与系统 `MediaCodec audio/opus` 解码，解码后复用现有 PCM jitter buffer / AudioTrack。
- Windows 桌面端 Opus 选择会明确进入 V2 Header 实验路径；切回 Legacy 会自动切回 PCM16。
- 默认仍为 `legacy_las1 + pcm16`；Opus 尚未完成真机验收，当前只能作为实验能力。
...
这些指标用于定位链路时钟/队列/帧边界问题；Opus 虽已有实验接线，但仍需单独真机验收，不代表已稳定替代 PCM16。
